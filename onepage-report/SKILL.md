...
#### 2.6.5 Citation Map 擴充（Web Search + AI 整理）
...
- **原文**：傳統方法透過拉高 CPU 頻率來維持 99% 不掉幀率...
- **補充說明**：
  - **術語解釋**：掉幀率指遊戲畫面無法在預期時間內渲染完成，導致畫面卡頓。
    99% 不掉幀率表示 100 幀中只允許 1 幀延遲。
  - **背景知識**：傳統做法是用「暴力拉頻」確保效能餘裕，但代價是高功耗和
    發熱。業界正轉向更智慧的調度方式，用預測取代暴力。
  - **參考來源**：[https://example3.com]
```
...
- Phase 2.6.5 已對每個 Citation 進行 web search 並整理了術語解釋
- 產生 glossary.md 時，優先從 Citation Map 的「補充說明 > 術語解釋」取用
- 這樣可確保術語解釋與原文脈絡一致，且已經過 web 查證
...
在嵌入 PPTX 之前，將所有 SVG 轉換為透明背景的 PNG。這樣可以：
- 確保圖表在 PowerPoint 中正確顯示
- 保留透明背景，讓圖表更好地融入投影片
- 避免 PowerPoint 對 SVG 的相容性問題

**轉換腳本（在 render_this.py 開頭執行）：**
...
def convert_svg_to_png(svg_path, png_path=None, scale=2):
    ...
    Args:
        svg_path: SVG 檔案路徑
        png_path: PNG 輸出路徑（預設為同名 .png）
        scale: 放大倍數（預設 2 倍，提高清晰度）
    """
    if png_path is None:
...
def convert_all_svg_to_png(output_dir='./output'):
    """
    將 output 目錄下所有 SVG 轉換為 PNG
    """
    png_files = {}
    for filename in os.listdir(output_dir):
        if filename.endswith('.svg'):
            ...
            png_files[filename] = png_path
            print(f"Converted: {filename} → {os.path.basename(png_path)}")
    return png_files


# === 在產生 PPTX 之前執行轉換 ===
png_files = convert_all_svg_to_png()
```
...
| 原始 SVG | 轉換後 PNG |
|----------|------------|
| `main_diagram.svg` | `main_diagram.png` |
| `appendix_diagram_1.svg` | `appendix_diagram_1.png` |
| `appendix_diagram_2.svg` | `appendix_diagram_2.png` |

**注意事項：**
- 使用 `scale=2` 確保 PNG 清晰度（2 倍解析度）
- `background_color=None` 保持透明背景
- 如果 SVG 有白色背景矩形，轉換後仍會是白色（非透明）
- 要達到透明效果，SVG 生成時不要加白色背景矩形

**SVG 生成時移除背景（讓 PNG 透明）：**

如果需要透明背景，在生成 SVG 時不要加背景矩形：
...
```python
# === 附錄投影片 - 流程圖詳解 ===
# 注意：使用 PNG 檔案（已在 Phase 6.2.5 從 SVG 轉換）
appendix_diagrams = [
    ("PC vs 手機同步機制", "./appendix_diagram_1.png", "比較兩平台的延遲控制機制"),
    ("Input-to-Display 延遲鏈路", "./appendix_diagram_2.png", "完整的觸控到顯示時序"),
    ("BufferQueue 緩衝機制", "./appendix_diagram_3.png", "Android 圖形管線的緩衝流程"),
    # ... 根據 diagrams.md 動態產生
]
...
點擊主投影片中的術語，會直接跳到附錄投影片（術語解釋頁）。

1. **解析 `[[術語]]` 標記**：
   - 掃描 one_page.md 中所有 `[[術語]]` 標記
   - 使用 `parse_text_with_terms()` 函數處理

2. **建立投影片內超連結**：
   - 使用 `pptx_reference.py` 中的 `add_term_hyperlink()` 函數
   - 連結目標：附錄投影片（術語解釋頁）
   - 術語文字以藍色底線標示（表示可點擊）
...
```python
from pptx_reference import (
    create_presentation, add_blank_slide, add_background,
    add_main_title, add_content_box_with_terms,
    parse_text_with_terms, add_term_hyperlink
)
...
# === 主投影片 (slide 0) ===
main_slide = add_blank_slide(prs)
add_background(main_slide, prs)

# === 附錄投影片 (slide 1) - 必須先建立，才能設定超連結目標 ===
appendix_slide = add_blank_slide(prs)
add_background(appendix_slide, prs)
add_main_title(appendix_slide, "附錄：術語解釋", "報告中專業術語的白話文解釋")

# 附錄投影片索引（用於超連結目標）
appendix_slide_index = 1

# === 在主投影片添加帶超連結的內容 ===
# 方式 1：使用 add_content_box_with_terms（推薦）
add_content_box_with_terms(
    main_slide, 0.25, 0.85, 6.0, 2.0,
    "技術關鍵點",
    [
        "若以下條件成立：",
        "• 降低跨 [[cluster]] 的 [[migration]]",
        "• 可減少 [[L2 cache]] miss",
        "→ 預估可改善 [[1% Low FPS]] 達 15%",
    ],
    appendix_slide_index, prs,
    title_color=ACCENT_GREEN
)

# 方式 2：手動處理術語標記
content_box = main_slide.shapes.add_textbox(Inches(0.25), Inches(3.0), Inches(6.0), Inches(1.0))
tf = content_box.text_frame
p = tf.paragraphs[0]

# 解析並處理術語標記
text = "降低跨 [[cluster]] 的 [[migration]]，可減少 [[L2 cache]] miss"
parse_text_with_terms(text, p, appendix_slide_index, prs, font_size=10)
...
- **引用於**：已驗證的成功要素 - 第 2 點
- **補充說明**：
  - **術語解釋**：掉幀率指遊戲畫面無法在預期時間內渲染完成...
  - **背景知識**：傳統做法是用「暴力拉頻」確保效能餘裕...
  - **參考來源**：[URL3]
