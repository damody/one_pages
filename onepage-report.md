...
選項：
1. L1 Executive - 高層主管，最少術語，聚焦決策
2. L2 Manager+Tech - 管理層+技術背景，允許少量技術詞（推薦）
3. L3 Technical - 技術人員，可包含更多條件/假設
```
...
選項：
1. E0 - 不要求新實驗，使用現有數據（預設）
2. E1 - 輕佐證，需要 1-2 個關鍵指標（如 FPS、功耗）
3. E2 - 強佐證，需要完整實驗設計與數據
```
...
選項：
1. Executive - 簡潔版，大字重點突出，適合高層（預設）
2. Technical - 技術版，資訊密集，多區塊詳細說明
3. POC Report - POC 報告，含實驗設計與成功判定準則
```
...
### Q1：{問題標題}
- **類型**：{missing_evidence | ambiguity | inconsistency | decision_risk}
- **問題**：{具體描述問題}
- **位置**：{標題/證據/影響/行動/圖表}
...
輸出檔案：
📊 ./output/one_page.pptx（一頁投影片）
📝 ./output/speaker_script.md（演講稿）
